...
class S4WC_Gateway extends WC_Payment_Gateway {
    ...
    protected function get_charge_description( $type = 'simple' ) {
        ...
        $charge_description = sprintf(
            __( 'Pago por %s (Orden: %s)', 'stripe-for-woocommerce' ),
            $product_name,
    ...
    protected function get_form_data() {
        ...
        if ( $this->order && $this->order != null ) {
		    ...
		    'billing_address1'  => 	$this->order->billing_address_1,
	...
	//-----------------------------------------------------------------------
	// Webservices del Banco Central Dolar
	$webservice = file_get_contents("http://api.marcos.do/central_bank_rates");
	$json = json_decode($webservice);
	$dolar = $json->dollar->buying_rate;

	//Total = Total / Dolar
	$d = (float) $dolar * 100;
	$total = (float) $this->form_data['amount'] / $d * 100;
	$total = (int) $total;
	
	// Custom Plataform Rojo
	$stripe_charge_data['amount']   = (float) $total;
        $stripe_charge_data['statement_description'] = get_bloginfo('wpurl');
	$stripe_charge_data['currency'] = 'usd';
        // Charge description
        $stripe_charge_data['description'] = $this->get_charge_description();
	$stripe_charge_data['receipt_email'] = $this->form_data['customer']['billing_email'];
	$stripe_charge_data['shipping']['name'] = $this->form_data['customer']['name'];
        $stripe_charge_data['shipping']['phone'] = $this->form_data['customer']['billing_phone'];
	$stripe_charge_data['shipping']['address']['line1'] = $this->form_data['customer']['billing_address1'];
	$stripe_charge_data['shipping']['address']['city'] = $this->form_data['customer']['billing_city'];
